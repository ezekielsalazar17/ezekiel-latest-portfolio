...
const SideBar = () => {
  ...
  return (
    <BlurFade delay={0.6}>
      <div className="w-full h-full lg:h-96 bg-background dark:bg-primary-foreground flex flex-col flex-wrap gap-4 p-6 md:p-8 rounded-md shadow-sm border border-gray-200 dark:border-accent">
          ...
          {experiences.map((experience, index) => (
            ...
            >
              <div className="flex flex-col items-center pt-2">
                ...
                {index < experiences.length - 1 && <div className="w-px flex-1 bg-muted-foreground/40 my-1" />}
              </div>
              ...
              <div className="flex flex-row w-full gap-3 pb-6">
                <div className="flex flex-col w-full">
                  ...
                  <div className="flex flex-row justify-between md:flex-col lg:flex-row w-full">
                    <h2 className="text-sm text-muted-foreground">{experience.company}</h2>
                    <h3 className="text-sm text-muted-foreground font-bold">{experience.duration}</h3>
                  </div>
                </div>
              </div>
            </div>
